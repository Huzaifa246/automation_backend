...
app.post("/api/generate-summary", async (req, res) => {
    try {
        ...
        let allContent = "";
        selectedBatch.articles.forEach((article) => {
        ...
        const contentChunks = chunkContent(allContent, MAX_TOKENS - MAX_RESPONSE_TOKENS - 1000 // Adjust to leave room for other parts
        );
        ...
        for (let i = 0; i < contentChunks.length; i++) {
            ...
            const tokenCount = countTokens(chunk);
            console.log(`Processing chunk ${i + 1} with token count: ${tokenCount}`);
            if (tokenCount > MAX_TOKENS) {
                throw new Error(`Chunk ${i + 1} exceeds the maximum allowed token count.`);
            }
            ...
            try {
                ...
                console.error(`Chunk ${i + 1}: Failed to parse JSON, attempting fallback.`, rawOutput);
                ...
                if (jsonMatch) {
                    try {
                        ...
                        console.error(`Chunk ${i + 1}: Fallback JSON parse failed.`, fallbackError);
                    }
                }
                else {
        ...
        const unique = (arr) => Array.from(new Set(arr.filter((item) => item.trim() !== "")));
        ...
        const finalSummary = {
            ...
            metadescription: unique(chunkSummaries.map((s) => s.metadescription || "")).join(",\n"),
    ...
    try {
        ...
        const response = await openai.chat.completions.create({
            ...
            messages: [
                {
                    role: "system",
                    content: "You are a professional summarizer specializing in Bitcoin that generates concise and engaging titles for articles.",
                },
                {
                    role: "user",
                    content: `Articles content to summarize:\n\n${content}`,
        ...
        const heading1 = content
            .substring(heading1Start + "Heading 1:".length, content.indexOf(",", heading1Start))
            .trim();
        ...
        if (!payload.heading1) {
            throw new Error("heading1 must be provided and should end at the first comma.");
        }
        ...
        const apiResponse = await fetch("https://edge21-backend.vercel.app/api/data/storeParaDetails", {
            method: "POST",
            headers: {
